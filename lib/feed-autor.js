...
/**
 * Perfis sociais (turista/profissional) por usuario_id — sem embed PostgREST (evita PGRST201).
 *
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {string[]} ids
 * @returns {Promise<Map<string, { username: string, nome: string, foto: string | null, role: string }>>}
 */
export async function fetchPerfisSociaisPorUsuarioIds(supabase, ids) {
  /** @type {Map<string, { username: string, nome: string, foto: string | null, role: string }>} */
  const map = new Map()
  const unique = [...new Set((ids || []).map((x) => String(x ?? '').trim()).filter(Boolean))]
  if (unique.length === 0) return map

  const [{ data: profRows }, { data: turRows }] = await Promise.all([
    supabase
      .from('profissionais')
  ...
  for (const p of profRows ?? []) {
    ...
    if (uid) profBy.set(uid, /** @type {Record<string, unknown>} */ (p))
  }

  for (const uid of unique) {
    const row = profBy.get(uid) || turBy.get(uid)
    if (!row) continue
    ...
    map.set(uid, {
      username: nomeUser || 'usuario',
      nome: nomeCompleto || nomeUser || 'Usuário',
      foto: pickFotoPerfilOuLegacy(row),
      role: profBy.has(uid) ? 'profissional' : 'turista',
    })
  }
  return map
}
...
export async function fetchNomeUsuarioParaStory(supabase, uid) {
  ...
  const role = userData?.role ?? null
  if (role === 'empresa') {
    const { data: perfil } = await supabase.from('empresas').select('nome_usuario').eq('usuario_id', uid).maybeSingle()
 ...
 * @param {unknown} u
 * @param {{ autorTipo?: string | null }} [opts]
 */
export function pickAutorDisplay(u, opts = {}) {
  ...
  if (temPerfilPlano) {
    ...
    const role = roleCol || 'user'
    const verificado =
      role === 'profissional'
        ? contaVerificadaDocumentacao('profissional', row)
        : role === 'empresa'
          ? contaVerificadaDocumentacao('empresa', row)
          : false
    return {
      nome,
      username,
  ...
  const autorTipoPost =
    opts.autorTipo != null && String(opts.autorTipo).trim() !== ''
      ? String(opts.autorTipo).trim().toLowerCase()
      : null

  const publicarComoEmpresa =
    autorTipoPost === 'empresa' || (autorTipoPost == null && roleCol === 'empresa')

  /** Conta empresa na view pode ter também linha em turistas/profissionais. */
  if (publicarComoEmpresa && empresa) {
  ...
  if (roleCol === 'profissional' && profissional) {
    ...
    return {
      ...
      empresa_id: '',
      role: 'profissional',
      verificado: contaVerificadaDocumentacao('profissional', profissional),
    }
  }
  if (turista) {
    ...
    return { nome, username, foto_perfil_url: foto, usuario_id: usuarioId, empresa_id: '', role: 'turista', verificado: false }
  }
  const empresaSomenteAnfitriao = empresa?.somente_anfitriao === true
  if (empresa && !empresaSomenteAnfitriao) {
    const nome = String((empresa.nome_fantasia ?? email) || 'Empresa')
    const username = String(
      (typeof row.username === 'string' && row.username.trim() !== '' ? row.username.trim() : null) ??
        empresa.nome_usuario ??
