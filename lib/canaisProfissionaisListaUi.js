...
const COMUNIDADE_TO_SLUG = {
  ...
  'Motoristas de App': 'motorista_app',
  'Motoristas de APP': 'motorista_app',
  ...
  Vans: 'van',
  Anfitrião: 'anfitriao',
  'Anfitrião / Hospedagem': 'anfitriao',
  Anfitrioes: 'anfitriao',
  Anfitriao: 'anfitriao',
  Anfitriões: 'anfitriao',
...
/**
 * Prefixo legado em canais de empresa: "Guia - Nome Fantasia", "Motoristas de App - …".
 * @param {string | null | undefined} nome
 */
function prefixoLegadoNomeCanalEmpresa(nome) {
  const raw = String(nome ?? '').trim()
  const sep = raw.indexOf(' - ')
  if (sep <= 0) return ''
  const prefix = raw.slice(0, sep).trim()
  const norm = prefix
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toUpperCase()
  if (norm === 'ADM' || norm === 'FINANCEIRO' || norm === 'TURISTA') return ''
  return prefix
}

/**
 * Slug da comunidade profissional a partir de um canal de empresa.
 * @param {{ categoria?: string | null; nome?: string | null; comunidade_prof?: string | null }} c
 */
export function slugComunidadeProfissionalDeCanalEmpresa(c) {
  return chaveProfissionalCanal(c)
}
...
export function chaveProfissionalCanal(c) {
  ...
  const legado = prefixoLegadoNomeCanalEmpresa(c.nome)
  if (legado) {
    const slugLegado = toSlugComunidadeProf(legado)
    if (slugLegado && COMUNIDADES_PROFISSIONAIS_SLUG.includes(slugLegado)) return slugLegado
  }
...
export function tituloPastaCanalColetivoProfissional(comunidadeOuSlug) {
  const slug =
    toSlugComunidadeProf(comunidadeOuSlug) ||
    chaveProfissionalCanal({ nome: comunidadeOuSlug, categoria: comunidadeOuSlug })
  // Dual mode: mesmo canal serve comunidade Anfitrião e empresa Hospedagem.
  if (slug === 'anfitriao') return 'Anfitrião / Hospedagem'
