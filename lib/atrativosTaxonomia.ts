...
import { normalizarTextoTaxonomia, type AtrativoCategoriaRow } from '@/lib/atrativosCatalogo'

/**
 * Resolve ou cria sessão/categoria do catálogo de atrativos na empresa.
 */
export async function resolverOuCriarCategoriaAtrativo(
  ...
  if (!norm) throw new Error('Informe a categoria (sessão) do atrativo.')

  const { data: existente, error: errSelect } = await supabase
    .from('atrativos_categorias')
    .select('id, nome')
...
export async function listarCategoriasAtrativoEmpresa(
  supabase: SupabaseClient,
  empresaId: string,
): Promise<AtrativoCategoriaRow[]> {
