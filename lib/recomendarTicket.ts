...
export type TicketRecomendacaoPersistencia = {
...
/**
 * Registra indicação de ticket/atrativo em `recomendacoes_ticket`.
 */
export async function registrarRecomendacaoTicket(
  supabase: SupabaseClient,
  params: TicketRecomendacaoPersistencia,
  ...
  const payload: Record<string, string> = {
    ...
    experiencia_id: params.experienciaId,
  ...
  let { error } = await supabase.from('recomendacoes_ticket').insert(payload)
  ...
  if (error && String(error.message ?? '').toLowerCase().includes('turista_')) {
    ...
    ;({ error } = await supabase.from('recomendacoes_ticket').insert(minimo))
  }
