 ...
 *   ehProfissionalAnfitriao: boolean
 *   erro: string | null
...
function profissionalEhAnfitriao(prof) {
  const slugs = Array.isArray(prof?.categorias) ? prof.categorias.map((c) => String(c).toLowerCase()) : []
  return slugs.some((s) => s === 'anfitriao' || s === 'anfitrião')
}
...
export async function fetchComissoesOfertasData(supabase, usuarioId, opts = {}) {
  ...
  const ehProfissionalAnfitriao = profissionalEhAnfitriao(profRes.data)
