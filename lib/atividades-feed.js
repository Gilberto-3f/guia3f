...
export async function enriquecerAtividadesEmpresaInterator(supabase, rows) {
  ...
  if (curtidaEmpresa.size === 0 && comentarioEmpresa.size === 0) return rows
...
/**
 * Minha conta — modo Anfitrião (perfil social): só interações no conteúdo pessoal/profissional.
 */
export function atividadeVisivelMinhaContaModoAnfitriao(row, usuarioLogadoId, ctx = {}) {
  if (atividadeFeitaComoEmpresaHospedagem(row)) return false
  if (!atividadeVisivelNaMinhaContaPessoal(row, usuarioLogadoId)) return false
  const alvo = atividadeAlvoEhConteudoEmpresa(row, ctx)
  if (alvo === true) return false
  return true
}

/**
 * Minha conta — modo Hospedagem: só interações no conteúdo da empresa.
 */
export function atividadeVisivelMinhaContaModoHospedagem(row, ctx = {}) {
 ...
 * @param {{ tipo?: string | null, autor_id?: string | null, alvo_id?: string | null }} row
 * @param {Record<string, PostMeta>} [postMap]
 */
export function chaveAtividadeCurtiuPost(row, postMap) {
  ...
  return `curtiu-post:${autor}:${canon}`
}
 ...
 * @param {Record<string, PostMeta>} postMap
 * @returns {(
 *   | { kind: 'curtiu_post_fotos', autor_id: string, usuario_dono_id: string, empresa_dono_id: string | null, donor_tipo: 'empresa' | 'usuario', rows: T[], created_at: string }
...
export function agruparAtividadesCurtidasPost(ordenadoDesc, postMap) {
  ...
  while (i < ordenadoDesc.length) {
    ...
    /** @type {Map<string, { dono: ReturnType<typeof resolverDonoPostAtividade>, rows: typeof grupo }>} */
    const porDono = new Map()
    for (const g of grupo) {
      const dono = donoDe(g)
      const cur = porDono.get(dono.chave)
      if (cur) cur.rows.push(g)
      else porDono.set(dono.chave, { dono, rows: [g] })
    }
    for (const { dono, rows: subRows } of porDono.values()) {
      for (let k = 0; k < subRows.length; k += 10) {
        const chunk = subRows.slice(k, k + 10)
        out.push({
