...
export function atividadeConteudoEhDoUsuario(row, usuarioLogadoId, ctx = {}, opts = {}) {
  ...
  // Marcação é destinada diretamente ao usuário marcado; o story pertence a quem marcou.
  // Portanto, não validar pelo autor do conteúdo nem separar pelos modos prof/empresa.
  if (tipo === 'marcou_em_story') return destId === uid
  ...
  if (tipo === 'curtiu_story' || tipo === 'repostou_story') {
...
/**
 * Minha conta — modo Hospedagem/Agência: só interações recebidas no conteúdo da empresa.
 * @param {{ tipo?: string | null, alvo_id?: string | null, dados_extras?: unknown } | null | undefined} row
 * @param {string} usuarioLogadoId
 * @param {{ postMetaMap?: Record<string, unknown>, storyMetaMap?: Record<string, { autor_tipo?: string | null }> }} ctx
 * @param {{ empresaIds?: string[] | Set<string> | null }} [opts]
 */
export function atividadeVisivelMinhaContaModoHospedagem(row, usuarioLogadoId, ctx = {}, opts = {}) {
  ...
  const idsRaw = opts.empresaIds
  // null/undefined = sem allowlist (ex.: conta role empresa).
  // [] = allowlist vazia → ocultar tudo (evita vazamento enquanto o id da agência/hospedagem não chegou).
  if (idsRaw == null) return true
  const idSet =
  ...
  const empId = empresaIdDoAlvoAtividade(row, ctx)
  if (empId) return idSet.has(empId)
  // Com allowlist de empresa: sem empresa_id no alvo → não mostrar (evita vazamento do perfil social).
  return false
}

/**
 * empresa_id do post/story alvo (quando resolvido no meta).
 * @param {{ tipo?: string | null, alvo_id?: string | null, dados_extras?: unknown } | null | undefined} row
 * @param {{ postMetaMap?: Record<string, unknown>, storyMetaMap?: Record<string, { autor_tipo?: string | null, empresa_id?: string | null }> }} ctx
 */
export function empresaIdDoAlvoAtividade(row, ctx = {}) {
  if (!row) return null
  const postMetaMap = /** @type {Record<string, PostMeta>} */ (ctx.postMetaMap ?? {})
  const storyMetaMap = ctx.storyMetaMap ?? {}
  const tipo = String(row.tipo ?? '')
  ...
  if (tipo === 'curtiu_post' || tipo === 'repostou_post') {
    const meta = postMetaCanonico(postMetaMap, row.alvo_id)
    if (meta?.empresa_id != null && String(meta.empresa_id).trim() !== '') {
      return String(meta.empresa_id).trim()
    }
    if (tipo === 'repostou_post') {
      const ex =
        row.dados_extras && typeof row.dados_extras === 'object' && !Array.isArray(row.dados_extras)
      ...
      const orig =
        ex.post_original_id != null && String(ex.post_original_id).trim() !== ''
          ...
          : ''
      if (orig) {
        const mo = postMetaMap[orig]
        if (mo?.empresa_id != null && String(mo.empresa_id).trim() !== '') {
          return String(mo.empresa_id).trim()
        }
      }
    }
    return null
  ...
  if (tipo === 'curtiu_story' || tipo === 'repostou_story') {
    const storyId =
      tipo === 'curtiu_story' ? storyIdDeAtividadeCurtiu(row) : storyIdDeAtividadeRepost(row)
    if (!storyId) return null
    const meta = storyMetaMap[storyId]
    if (meta?.empresa_id != null && String(meta.empresa_id).trim() !== '') {
      return String(meta.empresa_id).trim()
