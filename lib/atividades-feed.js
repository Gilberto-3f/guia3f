...
export function postEhCatalogo(p) {
  return postEhCatalogoProdutos(p) || postEhCatalogoCardapio(p) || postEhCatalogoServicos(p)
}
...
export function produtosSnapCatalogoPost(p) {
  ...
  const chave = Array.isArray(meta.servicos)
    ? 'servicos'
  ...
  const rotuloPadrao =
    chave === 'servicos' ? 'Serviço' : chave === 'pratos' ? 'Prato' : 'Produto'
...
/**
 * @param {PostMeta | undefined | null} p
 * @returns {'foto' | 'texto' | 'avaliacao' | 'repost' | 'verificacao_profissional' | 'catalogo_produtos' | 'catalogo_cardapio' | 'catalogo_servicos'}
 */
