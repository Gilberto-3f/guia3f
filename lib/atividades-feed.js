 ...
 * @param {{ operaComoEmpresaHospedagem?: boolean, ehAnfitriao?: boolean, role?: string | null, seguidosRede?: Set<string> | string[], autoresSomenteGuiaGlobal?: Set<string> | string[] }} [opts]
 */
export function atividadeVisivelNaAbaSeguindo(row, usuarioLogadoId, ctx = {}, opts = {}) {
  ...
  const seguidosRede =
    opts.seguidosRede instanceof Set
      ? opts.seguidosRede
      : new Set((opts.seguidosRede ?? []).map((id) => String(id).trim()).filter(Boolean))
  const autoresGuiaGlobal =
    opts.autoresSomenteGuiaGlobal instanceof Set
      ? opts.autoresSomenteGuiaGlobal
      : new Set((opts.autoresSomenteGuiaGlobal ?? []).map((id) => String(id).trim()).filter(Boolean))
  ...
  /**
   * Gestor de empresa no guia (global): interações no perfil social só para quem segue na rede.
   * Interações em modo hospedagem (empresa_interator_id) permanecem globais.
   */
  if (autoresGuiaGlobal.has(autorId) && !feitaComoEmpresa && !seguidosRede.has(autorId)) {
    return false
  }
...
export async function contarAtividadesMinhaContaNaoLidas(supabase, userId, opts = {}) {
  ...
  for (const r of candidatas) {
    if (r.tipo === 'curtiu_post' && r.alvo_id) postIds.push(String(r.alvo_id))
    const ex = r.dados_extras
    if (ex && typeof ex === 'object' && !Array.isArray(ex)) {
      const pid = /** @type {Record<string, unknown>} */ (ex).post_id
