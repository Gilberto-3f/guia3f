...
export function postEhCatalogo(p) {
  return postEhCatalogoProdutos(p) || postEhCatalogoCardapio(p)
}
...
export function produtosSnapCatalogoPost(p) {
  ...
  const chave = Array.isArray(meta.pratos) ? 'pratos' : 'produtos'
  const raw = Array.isArray(meta[chave]) ? meta[chave] : []
  const rotuloPadrao = chave === 'pratos' ? 'Prato' : 'Produto'
...
/**
 * @param {PostMeta | undefined | null} p
 * @returns {'foto' | 'texto' | 'avaliacao' | 'repost' | 'verificacao_profissional' | 'catalogo_produtos' | 'catalogo_cardapio'}
 */
