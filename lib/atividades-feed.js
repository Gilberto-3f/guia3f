...
export function atividadeVisivelNaAbaSeguindo(row, usuarioLogadoId, ctx = {}, opts = {}) {
  ...
  /** Interação recebida no meu conteúdo → só Minha Conta. */
  if (destId === uid) return false

  if (autorId === uid) {
    /** Modo anfitrião (perfil social): outbound feito como empresa não entra na aba Seguindo. */
    if (
      atividadeFeitaComoEmpresaHospedagem(row) &&
      opts.ehAnfitriao &&
