 ...
 * @returns {'foto' | 'texto' | 'avaliacao' | 'repost'}
 */
...
/**
 * Agrupa `curtiu_post` em blocos de fotos (máx. 10) por liker + dono + dia; demais tipos ficam soltos.
...
export function agruparAtividadesCurtidasPost(ordenadoDesc, postMap) {
  ...
  while (i < ordenadoDesc.length) {
    ...
    while (i < ordenadoDesc.length) {
      ...
      if (x.autor_id !== r.autor_id || x.usuario_id !== r.usuario_id || dayKey(x.created_at) !== dk) break
      grupo.push(x)
      i++
    }
