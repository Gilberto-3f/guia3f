...
/**
 * Contato WhatsApp a partir do detalhe de um ticket/atrativo.
 * @param {{ nomeTicket?: string, username?: string | null, ticketUrl?: string | null }} [opts]
 */
export function mensagemWhatsappTicket(opts = {}) {
  const nomeTicket = String(opts.nomeTicket ?? '').trim()
  const user = opts.username ? String(opts.username).replace(/^@/, '') : ''
  const handle = user ? `@${user}` : ''
  const ticketUrl = opts.ticketUrl ? String(opts.ticketUrl).trim() : ''

  const lines = []
  if (nomeTicket) {
    lines.push(
      `Olá${handle ? ` ${handle}` : ''}!\n\nVi o atrativo *${nomeTicket}* no Guia 3F e gostaria de mais informações.`,
    )
  } else {
    lines.push(
      `Olá${handle ? ` ${handle}` : ''}!\n\nVi os atrativos no Guia 3F e gostaria de mais informações.`,
    )
  }
  if (ticketUrl) {
...
/**
 * Recomendação de ticket/atrativo (profissional → turista).
 * @param {{
 *   ticketNome: string
 *   ticketUrl: string
 ...
 * }} p
 */
export function mensagemWhatsappRecomendacaoTicket({
  ticketNome,
  ticketUrl,
  precoInteira,
  precoMeia,
  ...
  const lines = [
    `Olá! Recomendo o atrativo *${ticketNome}* que está no Guia 3F...`,
    '',
    ticketUrl,
  ]

  const inteira =
    precoInteira != null && Number.isFinite(Number(precoInteira)) ? Number(precoInteira) : null
  const meia = precoMeia != null && Number.isFinite(Number(precoMeia)) ? Number(precoMeia) : null
  const precos = [
    inteira != null
      ? `Inteira R$ ${inteira.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
      : '',
    meia != null
      ? `Meia R$ ${meia.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
      : '',
  ]
    .filter(Boolean)
    .join(' · ')
