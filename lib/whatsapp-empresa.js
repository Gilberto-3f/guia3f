...
export function mensagemWhatsappRecomendacao({
  ...
  const lines = [`Olá! Recomendo *${empresaNome}* no Guia 3F:`, '', empresaUrl]
  ...
  const guia = profissionalUsername ? handlePagina(profissionalUsername) : ''
  if (guia) lines.push('', `Indicação do guia ${guia}`)
