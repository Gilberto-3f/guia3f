...
import { criarFetchAuthSerializado } from '@/lib/authTokenFetch'
...
function isRefreshTokenAlreadyUsed(err) {
  ...
  return (
    status === 409 ||
    /refresh.?token.?already.?used|Already Used|too many concurrent token refresh/i.test(msg) ||
...
function aplicarDedupeAuth(client) {
  ...
  /**
   * Nunca chama GET /auth/v1/user. JWT local (igual ao server).
   * Evita 504 "context deadline exceeded" quando o Auth/Postgres está saturado.
   * @returns {Promise<{ data: { user: import('@supabase/supabase-js').User | null }; error: null }>}
   */
    ...
    const runRefresh = () =>
      origRefresh(currentSession)
        ...
        .catch(async (err) => {
          ...
          // Duas abas/refresh paralelo (409 / already used) — espera o storage atualizar e relê
          if (isRefreshTokenAlreadyUsed(err)) {
            console.warn('[auth] refresh em conflito — relendo sessão local')
            cacheSession = null
...
export const supabase = createClient(supabaseUrl, supabaseAnonKey, {
  global: {
    fetch: criarFetchAuthSerializado({
      recoverSession: () => {
        const local = lerSessaoLocalStorage()
        const session = local?.data?.session
        if (!session || !jwtAindaValido(session)) return null
        return session
      },
    }),
  },
