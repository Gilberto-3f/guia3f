...
function isRefreshTokenAlreadyUsed(err) {
  ...
  return (
    /refresh.?token.?already.?used|Already Used|invalid.?refresh.?token/i.test(msg) ||
...
/**
 * Evita rajadas de GET /user (504) e faz fallback para sessão local em timeout do Auth.
  ...
  client.auth.refreshSession = async (currentSession) => {
    ...
    inflightRefresh = origRefresh(currentSession)
      .then((res) => {
        ...
        if (res?.data?.session) {
          cacheSession = { value: { data: { session: res.data.session }, error: null }, at: Date.now() }
        } else {
          cacheSession = null
        }
        return res
      ...
      .catch(async (err) => {
        ...
        // Duas abas/refresh paralelo: token já rotacionado — relê sessão local
