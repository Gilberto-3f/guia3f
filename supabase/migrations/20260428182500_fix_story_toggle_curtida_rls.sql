-- Permitir curtir stories via RPC sem liberar UPDATE geral em `stories`.
-- Mantém RLS ativa na tabela; a função roda como SECURITY DEFINER com row_security=off.
-- Importante: mantém RETURNS jsonb (StoryViewer depende de { liked, curtidas }).
...
BEGIN
  ...
  IF v_autor IS DISTINCT FROM v_uid THEN
    ...
    VALUES (
      ...
      p_story_id,
      'story',
      jsonb_build_object('story_id', p_story_id::text)
    );
  END IF;
