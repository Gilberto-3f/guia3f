    ...
    <article className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      <div className="flex gap-3 p-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {item.subcategoria_nome || item.marca_nome ? (
            <p className="mt-0.5 truncate text-xs text-gray-500">
              {[item.subcategoria_nome, item.marca_nome].filter(Boolean).join(' · ')}
            </p>
          ) : null}
          ...
          {item.categoria_nome ? (
            <p className="mt-0.5 text-[11px] text-gray-400">{item.categoria_nome}</p>
          ) : null}
        </div>
      </div>
      <div className="flex border-t border-gray-100">
        ...
        <button
          ...
          onClick={onExcluir}
          disabled={excluindo}
          className="flex flex-1 items-center justify-center gap-1.5 border-l border-gray-100 py-2.5 text-xs font-semibold text-red-600 hover:bg-red-50 disabled:opacity-50"
        >
