...
export type FormServicoState = {
  ...
  /** Texto livre da sessão (ex.: CABELEIREIRO) — resolve/cria em servicos_locais_categorias. */
  categoria: string
  descricao: string
  site_url: string
...
export function formServicoVazio(): FormServicoState {
  return {
    id: null,
    nome: '',
    preco_usd: '',
...
export function formServicoFromRow(
  row: ServicoLocalRow,
...
type Props = {
  form: FormServicoState
  onChange: (next: FormServicoState) => void
  onSalvar: () => void
  onCancelar: () => void
  salvando?: boolean
  titulo: string
      ...
      <div className="space-y-4">
        <label className={labelCls}>
          ...
          <input
            type="text"
            value={form.nome}
            onChange={(e) => onChange({ ...form, nome: e.target.value.slice(0, NOME_SERVICO_MAX) })}
            className={inputCls}
            maxLength={NOME_SERVICO_MAX}
          />
          <span className="mt-1 block text-right text-[10px] text-gray-400">
            {form.nome.length}/{NOME_SERVICO_MAX}
          </span>
        </label>
        ...
        <div className="rounded-lg border border-gray-100 bg-[#f5f5f5] p-3">
          ...
          {!temPrecoValido ? (
            <p className="mt-1.5 text-[11px] text-gray-500">
              Informe o valor do serviço para liberar o desconto.
            </p>
          ) : null}
        ...
        <label className={labelCls}>
          ...
          <span className="mt-1 block text-[10px] text-gray-400">
            Agrupa os serviços nesta sessão (como pastas da lista).
          </span>
        </label>
        ...
        <label className={labelCls}>
          ...
          <textarea
            ...
            onChange={(e) =>
              onChange({ ...form, descricao: e.target.value.slice(0, DESCRICAO_SERVICO_MAX) })
            }
            className={`${inputCls} min-h-[88px] resize-y`}
            maxLength={DESCRICAO_SERVICO_MAX}
            rows={3}
          />
          <span className="mt-1 block text-right text-[10px] text-gray-400">
            {form.descricao.length}/{DESCRICAO_SERVICO_MAX}
          </span>
        </label>
