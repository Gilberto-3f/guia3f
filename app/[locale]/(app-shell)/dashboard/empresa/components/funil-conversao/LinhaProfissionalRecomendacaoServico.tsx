...
import type { RecomendacaoServicoProfissional } from '../../types/dashboard.types'
...
interface Props {
  profissional: RecomendacaoServicoProfissional
  naoLidas?: number
  onAberto?: () => void
  posicao?: number
}

export default function LinhaProfissionalRecomendacaoServico({
  profissional,
  naoLidas = 0,
  onAberto,
  posicao,
  ...
  const resumo =
    total === 1 ? '1 serviço recomendado' : `${total.toLocaleString('pt-BR')} serviços recomendados`
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                {detalhe.servico_nome ? (
                  <p className="mt-1 truncate text-xs font-medium text-gray-500">{detalhe.servico_nome}</p>
                ) : null}
              </div>
              <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-lg bg-gray-100">
                {detalhe.servico_foto_url ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img
                    src={detalhe.servico_foto_url}
                    alt=""
