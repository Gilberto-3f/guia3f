      ...
      <div className="p-4">
        ...
        <div className="mt-4 border-t border-gray-100 pt-3">
          {paginasDocs.length === 0 ? (
            <p className="text-center text-xs text-gray-500">Nenhum documento anexado.</p>
          ) : (
            <>
              <button
                type="button"
                onClick={() => setDocsAbertos((v) => !v)}
                className="flex w-full items-center justify-between gap-2 rounded-lg py-2 text-left hover:bg-gray-50"
                aria-expanded={docsAbertos}
              >
                <span className="flex min-w-0 flex-1 items-center gap-2">
                  <span className="text-sm font-bold text-gray-900">Documentos anexados</span>
