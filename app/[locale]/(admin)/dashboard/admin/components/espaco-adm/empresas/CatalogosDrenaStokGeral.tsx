...
import AbaCatalogo from '@/app/[locale]/(app-shell)/dashboard/empresa/components/drena-stok/AbaCatalogo'
import { filtrarEmpresasCde } from '@/lib/empresasCdeAdm'
...
function CardEmpresaCatalogo({
  ...
  return (
    <article className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      ...
      {expandida ? (
        <div className="border-t border-gray-100 bg-gray-50/40 px-2 pb-3 pt-2">
          <AbaCatalogo empresaId={emp.id} />
        </div>
      ) : null}
    </article>
  )
...
export function CatalogosDrenaStokGeral({
  busca,
  onBuscaChange,
  empresas,
  loading,
  ...
  const empresasCde = useMemo(() => filtrarEmpresasCde(empresas), [empresas])

  return (
    <div className="space-y-4">
      <p className="text-center text-xs text-gray-500">
        Relatórios de catálogo das empresas de Ciudad del Este — mesma aba Catálogo do Drena-Stok de cada
        loja.
      </p>
      ...
      ) : empresasCde.length === 0 ? (
        <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50 py-10 text-center text-sm text-gray-500">
          {busca.trim().length >= 2
            ? 'Nenhuma empresa de CDE encontrada para esta busca.'
            : 'Nenhuma empresa de Ciudad del Este cadastrada.'}
        </div>
      ) : (
        <div className="space-y-3">
          {empresasCde.map((emp) => (
            <CardEmpresaCatalogo
              ...
              onToggle={() => setExpandidaId((atual) => (atual === emp.id ? null : emp.id))}
            />
          ))}
        </div>
      )}
