...
import { CATEGORIA_EMPRESA_HOSPEDAGEM_ANFITRIAO } from '@/lib/anfitriaoDualMode'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: prof } = await admin
      .from('profissionais')
      .select('id, categorias, empresa_hospedagem_id')
    ...
    const cats = Array.isArray(prof.categorias) ? prof.categorias.filter((c): c is string => typeof c === 'string') : []
    if (!categoriasIncluemAnfitriao(cats)) {
      return NextResponse.json({ error: 'not_anfitriao' }, { status: 403 })
    }

    if (prof.empresa_hospedagem_id) {
      return NextResponse.json({ error: 'already_registered' }, { status: 409 })
    }

    const { data: uCheck } = await admin.from('turistas').select('id').eq('nome_usuario', nomeUsuario).limit(1)
    const { data: pCheck } = await admin.from('profissionais').select('id').eq('nome_usuario', nomeUsuario).limit(1)
    ...
    const { data: empresa, error: empErr } = await admin
      ...
      .insert({
        ...
        categoria: CATEGORIA_EMPRESA_HOSPEDAGEM_ANFITRIAO,
        cidade,
        endereco,
        bairro: bairro || null,
        whatsapp,
    ...
    const { error: linkErr } = await admin
      .from('profissionais')
      .update({ empresa_hospedagem_id: empresaId })
      .eq('id', prof.id)
