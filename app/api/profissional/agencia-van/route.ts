...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: prof } = await admin
      .from('profissionais')
      .select('id, categorias, empresa_agencia_van_id, foto_perfil_url, foto_url')
    ...
    const cats = Array.isArray(prof.categorias)
      ...
      : []
    if (!categoriasIncluemVan(cats)) {
      return NextResponse.json({ error: 'not_van' }, { status: 403 })
    }

    if (prof.empresa_agencia_van_id) {
    ...
    const { data: empresa, error: empErr } = await admin
      ...
      .insert({
        ...
        categoria: CATEGORIA_EMPRESA_AGENCIA_VAN,
        cidade,
        endereco,
        bairro: bairro || null,
        whatsapp,
    ...
    const { error: linkErr } = await admin
      .from('profissionais')
      .update({ empresa_agencia_van_id: empresaId })
      .eq('id', prof.id)
