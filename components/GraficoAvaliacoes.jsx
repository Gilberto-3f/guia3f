  ...
  return (
    <div className={compact ? 'space-y-1' : 'space-y-2'}>
      {notas.map((nota) => {
        ...
        return (
          <div key={nota} className={`flex items-center ${compact ? 'gap-1.5' : 'gap-2'}`}>
            <span
              className={`inline-flex shrink-0 items-center gap-1.5 text-gray-600 ${compact ? 'w-12 text-xs' : 'w-14 text-sm'}`}
            >
              <span>{nota}</span>
