...
import PopupRecomendarServico from '@/components/servicos-locais/PopupRecomendarServico'
import { filtrarFavoritoIdsPorUsuario } from '@/lib/favoritosTurista'
import { mapServicoLocalRow, precoFinalUsd, SELECT_SERVICO_LOCAL, type ServicoLocalRow } from '@/lib/servicosLocaisCatalogo'
import { openWhatsAppChat, mensagemWhatsappServico } from '@/lib/whatsapp-empresa'
...
export default function DrawerServicosLocais({
  isOpen,
  onClose,
  empresaId,
  empresaNome,
  ...
  const [passo, setPasso] = useState<1 | 2>(() => (servicoIdInicial ? 2 : 1))
  ...
  const [favServicos, setFavServicos] = useState<Set<string>>(() => new Set())
  ...
  const abrirDiretoNoDetalhe = Boolean(servicoIdInicial)
  ...
  const carregar = useCallback(async () => {
    ...
    setCarregando(true)
    try {
      const [empRes, servicosRes, cotMap, sess] = await Promise.all([
        supabase
          .from('empresas')
          .select(
      ...
      const servicos = (servicosRes.data ?? []).map((r) => mapServicoLocalRow(r as Record<string, unknown>))
      ...
      if (uid && servicos.length) {
        ...
        setFavServicos(new Set())
      }

      if (servicoIdInicial) {
        const found = servicos.find((p) => p.id === servicoIdInicial)
        if (found) {
      ...
      console.error('[DrawerServicosLocais]', e)
      setSecoes([])
    } finally {
      setCarregando(false)
    }
  ...
  const abrirSite = () => {
    ...
    if (!url) {
      window.alert('Link do site não cadastrado para este servico.')
      return
    }
  ...
  const abrirWhatsapp = () => {
    ...
    const servicoUrl = origin
      ? `${origin}/servicos/item/${selecionado.id}?ref=whatsapp`
      : `/servicos/item/${selecionado.id}?ref=whatsapp`
    const ok = openWhatsAppChat(
      whatsappComercial,
      mensagemWhatsappServico({
        nomeServico: selecionado.nome,
  ...
  const totalServicos = useMemo(() => secoes.reduce((acc, s) => acc + s.servicos.length, 0), [secoes])
  ...
  return (
    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto" data-modal-scroll-lock-scrollable>
        {carregando ? (
          <p className="py-12 text-center text-sm text-gray-400">Carregando serviços…</p>
        ) : passo === 1 && !abrirDiretoNoDetalhe ? (
          <div className="space-y-5 pb-8 pt-3">
            {totalServicos === 0 ? (
              <p className="px-4 text-center text-sm text-gray-500">Nenhum servico disponível no momento.</p>
            ) : (
              secoes.map((sec) => (
                <section key={sec.categoriaId} className="space-y-2">
                  <h3 className="px-4 text-center text-sm font-bold text-[#0097b2]">
                    {sec.categoriaNome} • {sec.servicos.length}
                  </h3>
                ...
                {!perfilEhEmpresa ? (
                  <div className="flex flex-col items-center gap-1">
                    <BotaoEstrelaFavorito
                      ...
                      tipo="servico"
                      inicial={favServicos.has(selecionado.id)}
                      size={28}
