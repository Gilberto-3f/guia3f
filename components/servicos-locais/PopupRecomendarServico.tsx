...
import { registrarRecomendacaoServico } from '@/lib/recomendarServico'
import {
  montarTelefoneComDdi,
  PAISES_TELEFONE_RECOMENDACAO,
  paisTelefonePorId,
} from '@/lib/paisesTelefoneRecomendacao'
import { mensagemWhatsappRecomendacaoServico, openWhatsAppChat } from '@/lib/whatsapp-empresa'

export type ServicoRecomendacaoInfo = {
  id: string
  nome: string
...
type Props = {
  aberto: boolean
  onFechar: () => void
  servico: ServicoRecomendacaoInfo
}

function urlServicoRecomendacao(servicoId: string): string {
  const path = `/servicos/item/${servicoId}?ref=recomendacao`
  if (typeof window === 'undefined') return path
  return `${window.location.origin}${path}`
}

/** Recomendação de servico do serviços digital (perfil profissional) — conta no funil (aba SERVIÇOS). */
export default function PopupRecomendarServico({ aberto, onFechar, servico }: Props) {
  ...
  const montarMensagem = (profissionalUsername: string | null) =>
    mensagemWhatsappRecomendacaoServico({
      servicoNome: servico.nome,
      servicoUrl: urlServicoRecomendacao(servico.id),
      precoUsd: servico.precoUsd,
      precoBrl: servico.precoBrl,
      empresaNome: servico.empresaNome,
      empresaUsername: servico.empresaUsername,
  ...
  const enviar = async () => {
    ...
    try {
      if (modoContato === 'email') {
        ...
        const { profissionalUsername } = await registrarRecomendacaoServico(supabase, {
        ...
        const subject = encodeURIComponent(`Recomendação de servico: ${servico.nome}`)
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="popup-recomendar-servico-titulo"
      onClick={(e) => {
    ...
    >
      <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-3">
          <h3 id="popup-recomendar-servico-titulo" className="text-lg font-extrabold text-[#001f3f]">
            Recomendar servico
          </h3>
          <button
            type="button"
        ...
        <p className="mt-1 truncate text-sm text-gray-600">
          {servico.nome}
          {servico.empresaNome ? ` · ${servico.empresaNome}` : ''}
        </p>

        <label
          className="mt-4 block text-xs font-semibold text-gray-700"
          htmlFor={modoContato === 'whatsapp' ? 'contato-turista-rec-servico' : 'email-turista-rec-servico'}
        >
        ...
        {modoContato === 'whatsapp' ? (
          <div className="mt-1 flex gap-2">
            ...
            <input
              id="contato-turista-rec-servico"
              type="tel"
              inputMode="tel"
              autoComplete="tel"
