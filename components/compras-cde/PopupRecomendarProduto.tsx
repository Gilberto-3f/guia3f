...
import { mensagemWhatsappRecomendacaoProduto, openWhatsAppChat } from '@/lib/whatsapp-empresa'

export type ProdutoRecomendacaoInfo = {
  id: string
  nome: string
...
export default function PopupRecomendarProduto({ aberto, onFechar, produto }: Props) {
  ...
  const montarMensagem = (profissionalUsername: string | null) =>
    mensagemWhatsappRecomendacaoProduto({
      ...
      empresaUsername: produto.empresaUsername,
  ...
  const enviar = async () => {
    ...
    try {
      if (modoContato === 'email') {
        ...
        const subject = encodeURIComponent(`Recomendação de produto: ${produto.nome}`)
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="popup-recomendar-produto-titulo"
      onClick={(e) => {
    ...
    >
      <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-3">
          <h3 id="popup-recomendar-produto-titulo" className="text-lg font-extrabold text-[#001f3f]">
            Recomendar produto
          </h3>
          <button
            type="button"
        ...
        <p className="mt-1 truncate text-sm text-gray-600">
          {produto.nome}
          {produto.empresaNome ? ` · ${produto.empresaNome}` : ''}
        </p>

        <label
          className="mt-4 block text-xs font-semibold text-gray-700"
          htmlFor={modoContato === 'whatsapp' ? 'contato-turista-rec-produto' : 'email-turista-rec-produto'}
        >
        ...
        {modoContato === 'whatsapp' ? (
          <div className="mt-1 flex gap-2">
            ...
            <input
              id="contato-turista-rec-produto"
              type="tel"
              inputMode="tel"
              autoComplete="tel"
