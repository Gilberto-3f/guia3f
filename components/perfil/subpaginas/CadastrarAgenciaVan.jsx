...
const ERROS_API = {
  ...
  not_professional: 'Apenas profissionais podem cadastrar agência.',
  not_van: 'Esta opção é exclusiva para profissionais da categoria Motorista de Van.',
  already_registered: 'Você já possui uma agência cadastrada.',
  username_taken: 'Este nome de usuário já está em uso.',
  unauthorized: 'Faça login para continuar.',
}

/**
 * @param {{ onConcluido?: () => void }} props
 */
export default function CadastrarAgenciaVan({ onConcluido }) {
  ...
  const enviar = async (e) => {
    ...
    setErro('')
    try {
      const res = await fetch('/api/profissional/agencia-van', {
        method: 'POST',
      ...
      window.dispatchEvent(new Event('van-modo-refresh'))
  ...
  return (
    <form onSubmit={enviar} className="space-y-4 px-1 pb-6">
      <p className="rounded-xl bg-[#0097b2]/10 px-3 py-2 text-xs leading-relaxed text-gray-700">
        Cadastre sua agência no segmento Serviços Locais para criar pacotes e oferecer aos clientes.
        Após a aprovação, você poderá alternar entre Motorista de Van e Agência de Turismo.
      </p>
      ...
      <label className="block">
        ...
        <input
          ...
          placeholder="Ex.: Agência Van Turismo Triplice"
          required
        />
      </label>

      <CampoUsernameCadastro
        id="nomeUsuarioAgenciaVan"
        label="Nome de usuário *"
        value={nomeUsuario}
        onChange={setNomeUsuario}
        placeholder="sua_agencia"
      ...
      <CampoWhatsappCadastro id="whatsappAgenciaVan" label="WhatsApp *" onChange={setWhatsapp} />
      ...
      <label className="block">
        ...
        <textarea
          ...
          rows={3}
          className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm"
          placeholder="Apresente sua agência em poucas linhas"
        />
      </label>
      ...
      >
        {enviando ? 'Cadastrando…' : 'CADASTRAR AGÊNCIA'}
      </button>
    </form>
  )
}
