...
const ERROS_API = {
  ...
  not_guia: 'Esta opção é exclusiva para profissionais da categoria Guia de Turismo.',
 ...
 */
export default function CadastrarAgenciaGuia({ onConcluido }) {
  ...
  const enviar = async (e) => {
    ...
    setErro('')
    try {
      const res = await fetch('/api/profissional/agencia-guia', {
        method: 'POST',
      ...
      window.dispatchEvent(new Event('guia-modo-refresh'))
  ...
  return (
    <form onSubmit={enviar} className="space-y-4 px-1 pb-6">
      <p className="rounded-xl bg-[#0097b2]/10 px-3 py-2 text-xs leading-relaxed text-gray-700">
        ...
        Após a aprovação, você poderá alternar entre Guia de Turismo e Agência de Turismo.
      </p>
      ...
      <label className="block">
        ...
        <input
          ...
          placeholder="Ex.: Agência Trilha das Cataratas"
          required
        />
      </label>

      <CampoUsernameCadastro
        id="nomeUsuarioAgencia"
      ...
      <CampoWhatsappCadastro id="whatsappAgencia" label="WhatsApp *" onChange={setWhatsapp} />
