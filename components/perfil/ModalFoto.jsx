 ...
 * }} props
 */
export default function ModalFoto({ urls, indiceInicial, aberto, onFechar }) {
    ...
    <div className="fixed inset-0 z-[220] flex flex-col bg-black">
      <div className="flex items-center justify-between p-3 text-white">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={() => void baixar()} className="rounded-full p-2 hover:bg-white/10" aria-label="Download">
            <Download size={22} />
          </button>
          <button type="button" onClick={onFechar} className="rounded-full p-2 hover:bg-white/10" aria-label="Fechar">
            <X size={22} />
          </button>
        </div>
      </div>
      ...
      <div className="scrollbar-perfil flex gap-1 overflow-x-auto border-t border-white/10 p-2">
        {urls.map((u, idx) => (
          <button
            key={`${u}-${idx}`}
            type="button"
