...
import { registrarRecomendacaoPrato } from '@/lib/recomendarPrato'
import {
  montarTelefoneComDdi,
  PAISES_TELEFONE_RECOMENDACAO,
  paisTelefonePorId,
} from '@/lib/paisesTelefoneRecomendacao'
import { mensagemWhatsappRecomendacaoPrato, openWhatsAppChat } from '@/lib/whatsapp-empresa'

export type PratoRecomendacaoInfo = {
  id: string
  nome: string
...
type Props = {
  aberto: boolean
  onFechar: () => void
  prato: PratoRecomendacaoInfo
}

function urlPratoRecomendacao(pratoId: string): string {
  const path = `/cardapio/prato/${pratoId}?ref=recomendacao`
  if (typeof window === 'undefined') return path
  return `${window.location.origin}${path}`
}

/** Recomendação de prato do cardápio digital (perfil profissional) — conta no funil (aba CARDÁPIO). */
export default function PopupRecomendarPrato({ aberto, onFechar, prato }: Props) {
  ...
  const montarMensagem = (profissionalUsername: string | null) =>
    mensagemWhatsappRecomendacaoPrato({
      pratoNome: prato.nome,
      pratoUrl: urlPratoRecomendacao(prato.id),
      precoUsd: prato.precoUsd,
      precoBrl: prato.precoBrl,
      empresaNome: prato.empresaNome,
      empresaUsername: prato.empresaUsername,
  ...
  const enviar = async () => {
    ...
    try {
      if (modoContato === 'email') {
        ...
        const { profissionalUsername } = await registrarRecomendacaoPrato(supabase, {
          pratoId: prato.id,
          empresaId: prato.empresaId,
          categoriaId: prato.categoriaId,
          emailTurista: email,
        })

        const mensagem = montarMensagem(profissionalUsername)
        const subject = encodeURIComponent(`Recomendação de prato: ${prato.nome}`)
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="popup-recomendar-prato-titulo"
      onClick={(e) => {
    ...
    >
      <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-3">
          <h3 id="popup-recomendar-prato-titulo" className="text-lg font-extrabold text-[#001f3f]">
            Recomendar prato
          </h3>
          <button
            type="button"
        ...
        <p className="mt-1 truncate text-sm text-gray-600">
          {prato.nome}
          {prato.empresaNome ? ` · ${prato.empresaNome}` : ''}
        </p>

        <label
          className="mt-4 block text-xs font-semibold text-gray-700"
          htmlFor={modoContato === 'whatsapp' ? 'contato-turista-rec-prato' : 'email-turista-rec-prato'}
        >
