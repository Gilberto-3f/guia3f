...
import { registrarRecomendacaoTicket } from '@/lib/recomendarTicket'
import {
  montarTelefoneComDdi,
  PAISES_TELEFONE_RECOMENDACAO,
  paisTelefonePorId,
} from '@/lib/paisesTelefoneRecomendacao'
import { mensagemWhatsappRecomendacaoTicket, openWhatsAppChat } from '@/lib/whatsapp-empresa'

export type TicketRecomendacaoInfo = {
  id: string
  nome: string
...
type Props = {
  aberto: boolean
  onFechar: () => void
  ticket: TicketRecomendacaoInfo
}

function urlTicketRecomendacao(ticketId: string): string {
  const path = `/tickets/${ticketId}?ref=recomendacao`
  if (typeof window === 'undefined') return path
  return `${window.location.origin}${path}`
}

/** Recomendação de ticket/atrativo (perfil profissional) — conta no funil (aba TICKETS). */
export default function PopupRecomendarTicket({ aberto, onFechar, ticket }: Props) {
  ...
  const [modoContato, setModoContato] = useState<'whatsapp' | 'email'>('whatsapp')
  ...
  const montarMensagem = (profissionalUsername: string | null) =>
    mensagemWhatsappRecomendacaoTicket({
      ticketNome: ticket.nome,
      ticketUrl: urlTicketRecomendacao(ticket.id),
      precoInteira: ticket.precoInteira,
      precoMeia: ticket.precoMeia,
      empresaNome: ticket.empresaNome,
      empresaUsername: ticket.empresaUsername,
  ...
  const enviar = async () => {
    ...
    try {
      if (modoContato === 'email') {
        ...
        const { profissionalUsername } = await registrarRecomendacaoTicket(supabase, {
          experienciaId: ticket.id,
        ...
        const subject = encodeURIComponent(`Recomendação de atrativo: ${ticket.nome}`)
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="popup-recomendar-ticket-titulo"
      onClick={(e) => {
    ...
    >
      <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-3">
          <h3 id="popup-recomendar-ticket-titulo" className="text-lg font-extrabold text-[#001f3f]">
            Recomendar atrativo
          </h3>
          <button
            type="button"
        ...
        <p className="mt-1 truncate text-sm text-gray-600">
          {ticket.nome}
          {ticket.empresaNome ? ` · ${ticket.empresaNome}` : ''}
        </p>

        <label
          className="mt-4 block text-xs font-semibold text-gray-700"
          htmlFor={modoContato === 'whatsapp' ? 'contato-turista-rec-ticket' : 'email-turista-rec-ticket'}
        >
