...
export default function PostCard({
  ...
  if (tipoNorm === 'catalogo_cardapio') {
    const meta =
      post.avaliacao_meta && typeof post.avaliacao_meta === 'object' && !Array.isArray(post.avaliacao_meta)
        ...
        : {}
    const empresaCardapioId =
      meta.empresa_id != null && String(meta.empresa_id).trim() !== ''
        ? String(meta.empresa_id)
        : empresaId
    const pratosSnap = Array.isArray(meta.pratos) ? meta.pratos.slice(0, 3) : []
    const qtdMeta = Number(meta.quantidade) || pratosSnap.length || 0
    const textoCardapio =
      qtdMeta === 1
        ? 'cadastramos 1 novo prato em nosso cardápio, venha conferir.'
        : `cadastramos ${qtdMeta > 0 ? qtdMeta : 'XX'} novos pratos em nosso cardápio, venha conferir.`

    return (
      <article id={`feed-post-${post.id}`} className="rounded-xl bg-white shadow-sm">
        {cabecalhoAutorFeed}
        <div className="px-4 pb-3 pt-1">
          <p className="text-[15px] leading-snug text-gray-900">{textoCardapio}</p>
          {pratosSnap.length > 0 ? (
            <div
              className={`mt-3 flex items-stretch gap-2 overflow-x-auto pb-1 ${
                pratosSnap.length === 1 ? 'justify-center' : ''
              }`}
            >
              {pratosSnap.map((raw) => {
                const snap =
                  raw && typeof raw === 'object' && !Array.isArray(raw)
                    ...
                    : {}
                const nome = typeof snap.nome === 'string' ? snap.nome : 'Prato'
                const foto =
          ...
          {empresaCardapioId ? (
            <button
              type="button"
              onClick={() => setDrawerCardapioAberto(true)}
            ...
            >
              ...
              <span className="text-sm font-bold uppercase tracking-wide">CARDÁPIO</span>
            </button>
          ) : null}
        </div>
        ...
        {empresaCardapioId ? (
          <DrawerCardapio
            isOpen={drawerCardapioAberto}
            onClose={() => setDrawerCardapioAberto(false)}
            empresaId={empresaCardapioId}
  ...
  if (tipoNorm === 'catalogo_servicos') {
    const meta =
      post.avaliacao_meta && typeof post.avaliacao_meta === 'object' && !Array.isArray(post.avaliacao_meta)
        ? /** @type {Record<string, unknown>} */ (post.avaliacao_meta)
        : {}
    const empresaServicosId =
      meta.empresa_id != null && String(meta.empresa_id).trim() !== ''
        ? String(meta.empresa_id)
        : empresaId
    const servicosSnap = Array.isArray(meta.servicos) ? meta.servicos.slice(0, 3) : []
    const qtdMeta = Number(meta.quantidade) || servicosSnap.length || 0
    const textoServicos =
      qtdMeta === 1
        ? 'cadastramos 1 novo serviço em nosso catálogo, venha conferir.'
        : `cadastramos ${qtdMeta > 0 ? qtdMeta : 'XX'} novos serviços em nosso catálogo, venha conferir.`

    return (
      <article id={`feed-post-${post.id}`} className="rounded-xl bg-white shadow-sm">
        {cabecalhoAutorFeed}
        <div className="px-4 pb-3 pt-1">
          <p className="text-[15px] leading-snug text-gray-900">{textoServicos}</p>
          {servicosSnap.length > 0 ? (
            <div
              className={`mt-3 flex items-stretch gap-2 overflow-x-auto pb-1 ${
                servicosSnap.length === 1 ? 'justify-center' : ''
              }`}
            >
              {servicosSnap.map((raw) => {
                const snap =
                  raw && typeof raw === 'object' && !Array.isArray(raw)
                    ? /** @type {Record<string, unknown>} */ (raw)
                    : {}
                const nome = typeof snap.nome === 'string' ? snap.nome : 'Serviço'
                const foto =
                  snap.foto_url != null && String(snap.foto_url).trim() !== ''
                    ? String(snap.foto_url)
                    : null
          ...
          {empresaServicosId ? (
            <button
              type="button"
              onClick={() => setDrawerServicosAberto(true)}
              className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg bg-[#00D443] px-3 py-2.5 text-white transition-opacity hover:opacity-95"
              aria-label="Abrir serviços"
            >
              <Wrench className="h-4 w-4 shrink-0" aria-hidden />
              <span className="text-sm font-bold uppercase tracking-wide">SERVIÇOS</span>
            </button>
          ) : null}
        </div>
        <div className="border-t border-gray-100">{acoesPost}</div>
        ...
        {empresaServicosId ? (
          <DrawerServicosLocais
            isOpen={drawerServicosAberto}
            onClose={() => setDrawerServicosAberto(false)}
            empresaId={empresaServicosId}
            empresaNome={post.autor?.nome ?? post.autor?.username ?? 'Empresa'}
            empresaUsername={post.autor?.username ?? null}
            empresaFotoUrl={post.autor?.foto_perfil_url ?? null}
          />
  ...
  if (tipoNorm === 'catalogo_atrativos') {
    const meta =
      post.avaliacao_meta && typeof post.avaliacao_meta === 'object' && !Array.isArray(post.avaliacao_meta)
        ? /** @type {Record<string, unknown>} */ (post.avaliacao_meta)
        : {}
    const empresaAtrativosId =
      meta.empresa_id != null && String(meta.empresa_id).trim() !== ''
        ? String(meta.empresa_id)
        : empresaId
    const atrativosSnap = Array.isArray(meta.atrativos) ? meta.atrativos.slice(0, 3) : []
    const qtdMeta = Number(meta.quantidade) || atrativosSnap.length || 0
    const textoAtrativos =
      qtdMeta === 1
        ? 'cadastramos 1 novo atrativo em nosso catálogo, venha conferir.'
        : `cadastramos ${qtdMeta > 0 ? qtdMeta : 'XX'} novos atrativos em nosso catálogo, venha conferir.`

    return (
      <article id={`feed-post-${post.id}`} className="rounded-xl bg-white shadow-sm">
        {cabecalhoAutorFeed}
        <div className="px-4 pb-3 pt-1">
          <p className="text-[15px] leading-snug text-gray-900">{textoAtrativos}</p>
          {atrativosSnap.length > 0 ? (
            <div
              className={`mt-3 flex items-stretch gap-2 overflow-x-auto pb-1 ${
                atrativosSnap.length === 1 ? 'justify-center' : ''
              }`}
            >
              {atrativosSnap.map((raw) => {
                const snap =
                  raw && typeof raw === 'object' && !Array.isArray(raw)
                    ? /** @type {Record<string, unknown>} */ (raw)
                    : {}
                const nome = typeof snap.nome === 'string' ? snap.nome : 'Atrativo'
                const foto =
                  snap.foto_url != null && String(snap.foto_url).trim() !== ''
                    ? String(snap.foto_url)
                    : null
                const preco =
                  snap.preco_inteira != null && Number.isFinite(Number(snap.preco_inteira))
                    ? Number(snap.preco_inteira)
                    : snap.preco_meia != null && Number.isFinite(Number(snap.preco_meia))
                      ? Number(snap.preco_meia)
                      : null
                const idSnap = snap.id != null ? String(snap.id) : nome
                return (
                  <div
          ...
          {empresaAtrativosId ? (
            <button
              type="button"
              onClick={() => setDrawerAtrativosAberto(true)}
              className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg bg-[#00D443] px-3 py-2.5 text-white transition-opacity hover:opacity-95"
              aria-label="Abrir tickets"
            >
              <Ticket className="h-4 w-4 shrink-0" aria-hidden />
              <span className="text-sm font-bold uppercase tracking-wide">TICKETS</span>
            </button>
          ) : null}
        </div>
        <div className="border-t border-gray-100">{acoesPost}</div>
